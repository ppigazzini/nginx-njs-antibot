...
echo "== a configuration warning is written once =="

# Last, because restarting nginx makes a new shared zone and the counters
# above are read from the one this run filled.

# njs clones a VM per request, so a warning held in a module variable is
# written again on every request. The shared zone is what records that one
# was already written.
stop_nginx
: > "$TMP/logs/error.log"
start_nginx ANTIBOT_POW_BITS=999
for _ in $(seq 8); do curl -ks "$URL/" -o /dev/null; done
warned=$(grep -c 'POW_BITS=999' "$TMP/logs/error.log" || true)
[ "$warned" -eq 1 ] || fail "eight requests wrote $warned warnings for one setting, expected 1"
ok "one warning for POW_BITS=999 across eight requests"
stop_nginx
start_nginx
